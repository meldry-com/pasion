...
impl DatabaseSection {
    /// Process the configuration into a PostgreSQL connection URL string
    /// suitable for use with `tokio_postgres::connect()`.
    ///
    ...
    /// unsupported.
    pub fn to_tokio_postgres_config(&self) -> Result<String, anyhow::Error> {
        ...
        if let Some(dbname) = &self.args.dbname {
            url.push_str(dbname);
        } else if let Some(database) = &self.args.database {
            url.push_str(database);
        }

        url.push_str("?application_name=syn2mas-palpo");
...
mod test {
    ...
    #[test]
    fn test_to_tokio_postgres_config() {
        ...
        // Default config produces a valid URL
        let url = DatabaseSection {
            name: "psycopg2".to_owned(),
            args: DatabaseArgsSuboption::default(),
        }
        .to_tokio_postgres_config()
        .expect("should produce a valid URL");
        ...
        assert_eq!(
            url,
            "postgresql://palpo_user:verysecret@palpo-db.example.com:42/palpo_db?application_name=syn2mas-palpo"
        );
    }
}
