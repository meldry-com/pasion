...
impl WorkflowRepository for PgWorkflowRepository<'_> {
    ...
    async fn list_steps(
        ...
        workflow_steps::table
            .filter(
                workflow_steps::workflow_instance_id
                    .eq(Uuid::from(workflow_instance.id)),
            )
    ...
    async fn add_step(
        ...
        let sequence: i32 = params.sequence().try_into().map_err(|_| {
            DatabaseError::invalid_operation()
        })?;
    ...
    async fn set_step_status(
        ...
        let attempt_count: i32 = workflow_step.attempt_count.try_into().map_err(|_| {
            DatabaseError::invalid_operation()
        })?;

        let rows_affected = diesel::update(
            workflow_steps::table
                .filter(workflow_steps::id.eq(Uuid::from(workflow_step.id))),
        )
        .set((
