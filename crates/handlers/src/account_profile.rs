...
use crate::{passwords::PasswordManager, rest::Requester};
...
pub async fn deactivate_current_account(
    ...
    let password_ok = crate::account_password::verify_password_if_needed(
        requester.is_admin(),
    ...
    .map_err(|error| match error {
        crate::account_password::VerifyPasswordIfNeededError::Password(error) => {
            AccountProfileError::Password(error)
        }
        crate::account_password::VerifyPasswordIfNeededError::Repository(error) => {
