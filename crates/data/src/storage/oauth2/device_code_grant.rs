...
use pasion_data::{BrowserSession, Client, Clock, DeviceCodeGrant, Session};
...
pub trait OAuth2DeviceCodeGrantRepository: Send + Sync {
    ...
    ///
    /// [`Pending`]: pasion_data::DeviceCodeGrantState::Pending
    async fn fulfill(
        &mut self,
        clock: &dyn Clock,
    ...
    ///
    /// [`Fulfilled`]: pasion_data::DeviceCodeGrantState::Fulfilled
    async fn exchange(
        &mut self,
        clock: &dyn Clock,
        device_code_grant: DeviceCodeGrant,
