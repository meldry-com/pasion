...
/// Applies migrations once per test process before returning the pool.
///
/// # Panics
///
/// Panics if `DATABASE_URL` is unset, the connection pool cannot be built, or
/// migrations fail.
#[allow(
    clippy::unused_async,
...
pub async fn setup_test_pool() -> Pool<AsyncPgConnection> {
    ...
    MIGRATED
        .get_or_init(|| async {
            super::migrate(&pool, &database_url)
                .await
