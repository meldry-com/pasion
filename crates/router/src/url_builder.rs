...
impl UrlBuilder {
    ...
    pub fn redirect<U>(&self, destination: &U) -> salvo::writing::Redirect
    where
        U: Route,
    {
        let uri = self.relative_url_for(destination);
        salvo::writing::Redirect::other(&uri)
    }

    /// Create an absolute redirect response to a route
    pub fn absolute_redirect<U>(&self, destination: &U) -> salvo::writing::Redirect
    where
        U: Route,
    {
        let uri = self.absolute_url_for(destination);
        salvo::writing::Redirect::other(uri.as_str())
    }
