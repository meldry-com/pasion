...
For example, if `http.public_base` is `https://example.com/pasion/` and the provider ID is `01JABCDEF0123456789ABCDEFG`, the callback URL to register upstream is `https://example.com/pasion/upstream/callback/01JABCDEF0123456789ABCDEFG`.
...
The GitHub callback URL must use the same provider ULID as the `id` field below.
...
Notes:
 ...
 - `discovery_mode` must be `disabled` for GitHub
 - `subject` must be mapped explicitly to `{{ userinfo_claims.id }}` instead of the default `{{ user.sub }}`
 - GitHub may omit the email for some accounts, so `email.action: suggest` is usually safer than `require`
...
If your deployment lives under a path prefix, for example `https://example.com/pasion/`, the Google redirect URI must include that prefix as well.
