...
All database interactions are done through repositoriy traits. Each repository trait usually manages one type of data, defined in the [`pasion-data-model`][pasion-data-model] crate.
...
cd crates/storage-pg/ # Must be in the pasion-storage-pg crate folder
...
Migration files live in the `migrations` folder in the `pasion-storage-pg` crate.

```sh
cd crates/storage-pg/ # Again, in the pasion-storage-pg crate folder
export DATABASE_URL=postgresql:///matrix_auth
