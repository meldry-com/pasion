...
test_standard_scopes if {
	authorization_grant.allow with input.user as test_user
		with input.client as test_client
		with input.scope as ""
	...
	# Not supported yet
	not authorization_grant.allow with input.user as test_user
		with input.client as test_client
...
test_unstable_device_scopes if {
	...
	not authorization_grant.allow with input.client as test_client
		with input.grant_type as "client_credentials"
		with input.scope as "urn:matrix:org.matrix.msc2967.client:api:* urn:matrix:org.matrix.msc2967.client:device:AAbbCCdd01"
}
